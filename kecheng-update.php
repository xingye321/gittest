...
<?php
	...
	if ($kid == "null") {
		...
		include("conn.php");
		// 执行SQL语句
		$sql1 = "select 课程编号,课程名,时间 from kecheng where 课程编号={$kid}";
				...
				<form id="form1" action="kecheng-save.php" method="post" class="sui-form form-horizontal sui-validate">
					<div class="control-group">
						...
						<div class="controls">
							...
							<input type="text" value="<?php echo $row['课程名'] ?>" id="kcName" name="kcName" class="input-large input-fat" placeholder="请输入课程名称" data-rules="required|minlegth=2|maxlength=10">
						</div>
					</div>
					<div class="control-group">
						<label for="inputEmail" class="control-label">课程时间：</label>
						<div class="controls">
							<input type="text" value="<?php echo $row['时间'] ?>" id="kcTime" name="kcTime" class="input-large input-fat" data-toggle="datepicker" placeholder="请输入课程时间">
						</div>
					</div>
					<div class="control-group">
